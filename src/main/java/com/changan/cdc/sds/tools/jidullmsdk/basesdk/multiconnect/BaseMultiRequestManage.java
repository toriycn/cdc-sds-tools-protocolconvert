...
import com.changan.cdc.sds.tools.jidullmsdk.NetRequestUtil;
...
public class BaseMultiRequestManage {
    ...
    private BaseWebSocketMultiCommunicateResult innerConnectServer(String connectionSn) throws Exception {
        ...
        if(wsHashMap.containsKey(connectionSn)){
            llmWebScocketCommuniate =   wsHashMap.get(connectionSn);
            communicateResult.state = 1;
            communicateResult.communicate = llmWebScocketCommuniate;
        }
        else{
            llmWebScocketCommuniate = new BaseWebSocketMultiCommunicate(connectionSn,this);
            wsHashMap.put(connectionSn,llmWebScocketCommuniate);
            BaseWebSocketMultiCommunicate finalListener = llmWebScocketCommuniate;
            String baseUrl = getBaseUrl();
            if(getBaseUrl().contains("?")){
                 baseUrl += "&sn="+connectionSn;
            }
            else{
                baseUrl =String.format("%s?sn=%s",getBaseUrl(),connectionSn);
            }
            SDKLogUtil.log("BaseMultiRequestManage innerConnectServer:"+connectionSn+";baseUrl="+baseUrl);
    ...
    public int sendRequest(String connectionSn,Object requestJson){
        int state = 0 ;
        try{
            SDKLogUtil.log("BaseMultiRequestManage client sendRequest:"+connectionSn+";requestJson="+requestJson);
            BaseWebSocketMultiCommunicateResult communicateResult = innerConnectServer(connectionSn);
            if(communicateResult.state == 1 ||communicateResult.state == 2 ){
                state = communicateResult.communicate.sendOneMsgToServer(requestJson);
                //如果服务还未ready
                if(state !=2){
                    SDKLogUtil.log("BaseMultiRequestManage client send pending, sendRequest:"+connectionSn+";requestJson="+requestJson);
                }
            }
            else{
                SDKLogUtil.log("BaseMultiRequestManage client send state error,state"+ communicateResult.state+";sendRequest:"+connectionSn+";requestJson="+requestJson);
            }
            selector.wakeup();
        }catch (Exception ex){
            SDKLogUtil.log("BaseMultiRequestManage client sendRequest:"+connectionSn+";requestJson="+requestJson+";exception:"+ex.getMessage());
            ex.printStackTrace();
        }
        return state;
    }
